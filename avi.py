...
class VoiceDeepfakeDetector:
    ...
    def __init__(self):
        ...
        # AI model (low_cpu_mem_usage keeps peak RAM down during load,
        # which matters a lot on memory-constrained free hosting tiers)
        self.model = (
            AutoModelForAudioClassification
            .from_pretrained(
                MODEL_NAME,
...
def load_required_model(spinner_text, loader_fn, model_label):
    """
    Load a model with a clean, user-facing error instead of a raw
    traceback if the download/load fails (network hiccup on the
    hosting platform, out-of-memory, missing dependency, etc.).
    """
    try:
        ...
        st.error(
            f"❌ Could not load the {model_label}. "
            f"This is usually a network issue while downloading the "
            f"model, or the host running out of memory.\n\n"
            f"Details: {error}"
        )
        st.stop()


detector = load_required_model(
    "Loading AI detection model...",
    load_detector,
    "voice deepfake detection model"
)

language_detector = load_required_model(
    "Loading language identification model...",
    load_language_detector,
    "language identification model"
)

asr_pipeline = load_required_model(
